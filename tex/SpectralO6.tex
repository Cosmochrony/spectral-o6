...
\begin{document}
  ...
  \begin{abstract}
    ...
    masses.
    O4 derived the upper bound $\beta \leq 1$ within its growing-degree LPS closure from
    bounded Born--Infeld flux.
    ...
    space of growing dimension $O(q^2)$ as the next candidate class.
...
\item[O4~\cite{Beau2026a8}] Shows, within the growing-degree LPS closure used there, that the
...
short for reliable exponent extraction.
      Proposes a \emph{dynamical redundancy} mechanism in a matrix-level transition space of
      growing dimension $O(q^2)$ as the next candidate.
    \end{description}

    O5 thus isolates a candidate class of mechanisms and defers construction to the
    present paper.
...
$|S^*|/|G| = O(q^2/q^3) = O(1/q) \to 0$ as $q \to \infty$.
    ...
    \begin{remark}[Pre-saturation window length]
      ...
      The dimension count gives only the upper bound $|S^*|=O(q^2)$, compared with an
      $O(q)$ ambient bound for the Steinberg fingerprint of O5.
      On $X_{5,q}$, this upper bound is $O(q^2)$ against
      $|G|=q(q^2-1)/2\approx q^3/2$.
      It does not imply that the BFS-depth pre-saturation window grows with $q$.
      The measurements in Section~\ref{ssec:saturation} instead show saturation at depth~6
      for every tested prime, which is too short for a stable log--log exponent extraction.
    \end{remark}
  ...
  \subsection{Effective frontier size and the modified growth equation}
    ...
    \begin{remark}[Scope of Section~\ref{sec:suppression}]
      ...
      It instead provides a conditional consequence for the LPS cascade: if a fingerprint in
      dimension $O(q^{2k})$ satisfies a power-law decay of its redundancy functional
      (Hypothesis~\ref{hyp:power-law}), then the growth of $p(n)$ is sub-diffusively
      suppressed according to the O4 growth equation.
      The conclusion falls within the phenomenological window only when the fitted exponent
      also lies in the interval stated in Proposition~\ref{prop:beta-eff}.
      ...
      satisfy.
      It does not transfer this equation to the fixed-degree Heisenberg Cayley cascade; that
      cross-substrate step has no native growth carrier in the frozen corpus~\cite{Beau2026sgn}.
    \end{remark}

    We model the productive LPS frontier by inserting a hypothetical matrix-level redundancy
    multiplicatively into the O4 growth equation:
    \begin{equation}
      \label{eq:eff-growth}
      \frac{dp}{dn} \lesssim \Rn \cdot c_{\mathrm{BI}} \sqrt{p(n)},
    \end{equation}
    where the insertion of $\Rn$ is a modelling assumption identifying it with the productive
    fraction of the LPS
    boundary, not a definition valid on an arbitrary substrate.
    ...
    \begin{remark}[Comparison with O5's formula]
      ...
      (Proposition~\ref{prop:Rn-decay}).
      The numerical measurements in Section~\ref{sec:numerics} do not yield a stable
      power-law exponent: the bounded pre-saturation window prevents that extraction.
    \end{remark}
...
approaches $\dim W \cdot \dim \mathrm{Ad}(u)$.
    ...
    This geometric saturation-of-directions mechanism motivates testing decay faster than a
    random-matrix model would predict.
    The present bounded-depth data do not define a stable $\alpha$ and therefore do not test the
    required interval quantitatively.
  ...
  \subsection{Confinement of the exponent}
    ...
    This gives a conditional LPS-side mechanism by which the phenomenological window $\beta^*$
    can arise, \emph{provided} a fingerprint with a sufficiently long
  ...
  \subsection{Comparison across O4--O6}
    ...
    \begin{table}[ht]
      ...
      \begin{tabular}{@{}lllp{5cm}@{}}
        ...
        \midrule
        O4    & Bounded flux + Cheeger & $O(1)$      & $\beta \leq 1$ (LPS closure) \\
    ...
    finite-dimensional representation can produce the required scaling on this LPS cascade.
...
\item Retained, for the LPS cascade and the multiplicative productive-frontier model, the scaling
formula $\beff = 1/(1/2 + \alpha)$ (Proposition~\ref{prop:beta-eff}).
It applies to fingerprints satisfying Hypothesis~\ref{hyp:power-law} and will serve as the basis for
...
requires identifying the correct group-theoretic embedding.
  ...
  \subsection{Honest assessment}
    ...
    Even if such a regime is constructed on the LPS family, applying its exponent equation to
    a fixed-degree Heisenberg cascade would remain a separate cross-substrate hypothesis; O6
    supplies no native Heisenberg growth carrier~\cite{Beau2026sgn}.
    ...
    More broadly, Proposition~\ref{prop:bounded-depth} implies that the LPS-side cascade exponent
    $\beta$ cannot be generated within any fixed finite-dimensional representation framework,
    regardless of the representation used within that construction.
