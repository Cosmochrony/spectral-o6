...
$|\partial_t \chi_v| \leq c_\chi$, inherited from Born--Infeld saturation~\cite{Beau2026c},
forces $\Delta p(n) \lesssim c_\chi \sqrt{p(n)}$, which integrates to $p(n) \lesssim C n^2$.
...
$|S^*|/|G| = O(q^2/q^3) = O(1/q) \to 0$ as $q \to \infty$.
  ...
  \subsection{Effective frontier size and the modified growth equation}
    ...
    \begin{equation}
      \label{eq:eff-growth}
      \frac{dp}{dn} \lesssim \Rn \cdot c_\chi \sqrt{p(n)},
    \end{equation}
    ...
    \begin{proof}
      ...
      \[
        \frac{dp}{dn} \;\lesssim\; c_\chi \, p(n)^{1/2 - \alpha}.
      \]
      Separating variables: $p^{\alpha - 1/2} dp \lesssim c_\chi \, dn$, so
      $p(n)^{\alpha + 1/2} \lesssim (\alpha + 1/2) c_\chi n + \text{const}$,
...
approaches $\dim W \cdot \dim \mathrm{Ad}(u)$.
  ...
  \subsection{BFS step profile and saturation}
    ...
    \begin{table}[ht]
      ...
      \caption{BFS step profile for the Steinberg-matrix fingerprint on $X_{5,q}$.
        $d_{\rm St} = (q+1)^2$ is the ambient dimension.
        $|S^*|$ is the first $|S_n|$ at which $\rbmat_n = 0$.}
      \label{tab:saturation}
      \begin{tabular}{@{}rrrrrrr@{}}
        \toprule
        $q$ & $|G|$ & $d_{\rm St}$ & $\rbmat$ at step 4 & Sat.\ step & $|S^*|$ &
        $|S^*|/|G|$ \\
        \midrule
    ...
    Second, the Steinberg module is fully saturated (rank $= d_{\rm St}$) at step
  ...
  threshold} ($|S^*|/|G|$ is not small for $q=29$).
    ...
    \begin{figure}[ht]
      \centering
      \includegraphics[width=0.95\textwidth]{fig_O6_main.pdf}
      ...
      \textbf{Bottom left:} Cumulative admissible front $p_n^{\rm eff}$ vs.\ $|S_n|$
        (log-log), showing the saturation at $d_{\rm St} = (q+1)^2$.
...
requires identifying the correct group-theoretic embedding.
  ...
  where $C_0 = c_\chi \cdot A$ absorbs the $O(1)$ constants.
...
def run_cascade(generators, steinberg_maps, max_steps=None):
  ...
  \begin{thebibliography}{99}
    ...
    \bibitem{Beau2026a7}
    J.~Beau,
    ``Projective Dynamics and Mass Hierarchy: Hierarchical Amplification via Growing
    Relational Valence,''
    Zenodo (2026).
    ...
    \bibitem{Beau2026a8}
    J.~Beau,
    ``Subdiffusive Valence Growth under Bounded Relational Flux: Structural Derivation
    of the Cascade Exponent $\beta$,''
    Zenodo (2026).
    ...
    \bibitem{Beau2026a9}
    J.~Beau,
    ``Admissible Frontier Saturation and the Cascade Exponent: A Representation-Theoretic
    Obstruction and its Matrix-Level Refinement,''
    Zenodo (2026).
    DOI: \href{https://doi.org/10.5281/zenodo.19101472}{0.5281/zenodo.19101472}.
    ...
    \textit{Bull.\ Amer.\ Math.\ Soc.\ } \textbf{43}(4) (2006), 439--561.
